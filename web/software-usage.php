...
# Revision info:
# $HeadURL: http://svn.osc.edu/repos/pbstools/releases/pbstools-2.0/web/software-usage.php $
# $Revision: 236 $
	...
	if ( $key!='system' && $key!='start_date' && $key!='end_date' )
	  {
	    ...
	    $sql = "SELECT system, COUNT(jobid) AS jobcount, SUM(nproc*TIME_TO_SEC(walltime))/3600.0 AS cpuhours, SUM(TIME_TO_SEC(cput))/3600.0 AS cpuhours_alt, COUNT(DISTINCT(username)) AS users, COUNT(DISTINCT(groupname)) AS groups FROM Jobs WHERE system LIKE '".$_POST['system']."' AND ( ";
	    ...
	    $sql .= " ) AND ( ".dateselect('start',$_POST['start_date'],$_POST['end_date'])." ) GROUP BY system";
	    if ( $_POST['system']=="%" )
	      {
# compute totals iff wildcarding on all systems
		$sql .= " UNION SELECT 'TOTAL:',COUNT(jobid) AS jobcount, SUM(nproc*TIME_TO_SEC(walltime))/3600.0 AS cpuhours, SUM(TIME_TO_SEC(cput))/3600.0 AS alt_cpuhours, COUNT(DISTINCT(username)) AS users, COUNT(DISTINCT(groupname)) AS groups FROM Jobs WHERE ( ";
		if ( isset($pkgmatch[$key]) )
		  {
		    $sql .= $pkgmatch[$key];
		  }
